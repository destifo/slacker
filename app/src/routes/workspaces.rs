...
use crate::{
    core::state::AppState, 
    handlers::workspaces::{
        get_active_workspace, get_workspace_settings, link_workspace, list_workspaces, 
        reset_emoji_mappings, setup_workspace, switch_workspace, unlink_workspace,
        update_emoji_mappings, update_workspace_tokens,
        get_workspace_users, invite_user_to_workspace, remove_user_from_workspace,
    }
};
...
pub fn workspace_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/:workspace_name/emoji-mappings", put(update_emoji_mappings))
        .route("/:workspace_name/emoji-mappings/reset", post(reset_emoji_mappings))
        // User management routes
        .route("/:workspace_name/users", get(get_workspace_users))
        .route("/:workspace_name/users/invite", post(invite_user_to_workspace))
        .route("/:workspace_name/users/remove", post(remove_user_from_workspace))
}
