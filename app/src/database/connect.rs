...
pub async fn connect_database() -> Result<DatabaseConnection, DatabaseError> {
    let database_url = env::var("DATABASE_URL")
        .map_err(|e| DatabaseError::EnvError(format!("Database URL is missing from env, {}", e)))?;
    ...
    let max_connections: u32 = env::var("MAX_DB_CONNECTIONS")
        .map(|val| val.parse().unwrap_or(10))
        .unwrap_or(10);
    let min_connections: u32 = env::var("MIN_DB_CONNECTIONS")
        .map(|val| val.parse().unwrap_or(2))
        .unwrap_or(2);
